...
class Consumer
  ...
  def buy(type)
    until @demand <= 0 or Market.supply(type) <= 0
      cheapest_producer = Market.cheapest_producer(type)
      if cheapest_producer
        @demand *= 0.5 if cheapest_producer.price[type] > MAX_ACCEPTABLE_PRICE[type]
        cheapest_supply = cheapest_producer.supply[type]
        if @demand > cheapest_supply
          @demand -= cheapest_supply
          cheapest_producer.supply[type] = 0
        else
          cheapest_producer.supply[type] -= @demand
          @demand = 0
        end
      end
    end
...
class Market
  ...
  def self.demand
    $consumers.inject(0) { |memo, consumer| memo + consumer.demand}
  end
...
SIMULATION_DURATION.times do |t|
  $consumers.each do |consumer|
    consumer.demand = $generated_demand[t]
  end
